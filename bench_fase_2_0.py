...
def run_benchmark(num_files=100):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"[2] Escaneo completado: {len(archivos)} archivos, {len(carpetas)} carpetas en {scan_time:.4f}s"
        )
        ...
        print(
            f"[3] Agrupación completada: {len(grupos)} grupos con mismo tamaño en {group_time:.4f}s"
        )

        # 3. Hashing (con thread)
        start_hash = time.time()
        hashes = calcular_hash_grupo(
            archivos[: min(50, len(archivos))]
        )  # Limitar para evitar timeouts en tests
        hash_time = time.time() - start_hash
        print(
            f"[4] Hashing completado: {len(hashes)} hashes en {hash_time:.4f}s (con ThreadPoolExecutor)"
        )
